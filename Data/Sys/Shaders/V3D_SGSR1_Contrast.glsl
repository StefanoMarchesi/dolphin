// SGSR1 with multi-channel, contrast-relative edge detection for V3D.
// Filtering remains identical to the stock SGSR1 presentation shader.
...
void main()
{
  ...
  float4 left_r = textureGather(samp0, float3(coord, GetLayer()), 0);
  float4 left_g = textureGather(samp0, float3(coord, GetLayer()), 1);
  float4 left_b = textureGather(samp0, float3(coord, GetLayer()), 2);
  float4 left_luma = left_r * 0.2126 + left_g * 0.7152 + left_b * 0.0722;
  float center_luma = dot(color.xyz, float3(0.2126, 0.7152, 0.0722));
  float3 channel_vote = float3(
      abs(left_r.z - left_r.y) + abs(color.r - left_r.y) + abs(color.r - left_r.z),
      abs(left_g.z - left_g.y) + abs(color.g - left_g.y) + abs(color.g - left_g.z),
      abs(left_b.z - left_b.y) + abs(color.b - left_b.y) + abs(color.b - left_b.z));
  float edge_vote = max(channel_vote.r, max(channel_vote.g, channel_vote.b));
  float local_luma = (dot(left_luma, float4(1.0)) + center_luma) * 0.2;
  float relative_threshold = max(2.0 / 255.0, V3D_EDGE_THRESHOLD * (0.5 + local_luma));

  if (edge_vote > relative_threshold)
  {
    const int channel = 1;
    float4 left = left_g;
